...
/*** defines ***/
#define KANO_VERSION "0.0.1"
#define KANO_TAB_STOP 8

// get CTRL+k, for example, q is 113, crtl+q is 17, 113 & 0x1f = 17
#define CTRL_KEY(k) ((k) & 0x1f) 

#define Tip_Text "HELP: Ctrl-Q = quit"
...
int getWindowSize(int *rows, int *cols) {
  ...
  // TIOCGWINSZ(Terminal I/O Control Get WINdow SiZe) tell ioctl get terminal size;
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (KANO_TAB_STOP - 1) - (rx % KANO_TAB_STOP);
    rx++;
  }
  return rx;
}
...
void abAppend(struct abuf *ab, const char *s, int len) {
  char *c = realloc(ab->b, ab->len + len);
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
            "Kano editor -- version %s", KANO_VERSION);
...
int main (int argc, char *argv[]) {
  ...
  editorSetStatusMessage(Tip_Text);
